...
class MemoryOp(TACOp):
    ...
    def __post_init__(self):
        if self.target is None:
            raise ValueError(f"Memory operation {self.op_type} requires a target address")
...
class ArrayManager:
    ...
    def access_array(self, base_addr: int, index: Union[int, str], offset: int = 0) -> List[TACOp]:
    ...
    def store_array(self, base_addr: int, index: Union[int, str], value: int, 
                   offset: int = 0) -> List[TACOp]:
